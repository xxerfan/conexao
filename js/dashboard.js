...
function _drawEmptyChart(ctx, msg) {
  ...
  const c = ctx.getContext ? ctx : ctx;
  // Não desenha nada — apenas mostra texto centralizado via plugin
  new Chart(ctx, {
    type: 'doughnut',
...
function buildMetasSummary(metas) {
  ...
  <div class="card mb-3">
    <div class="card-header">
      <h3><i class="fas fa-bullseye"></i> Metas — Execução Física</h3>
    ...
    <div class="table-wrapper">
      <table>
        <thead>
          <tr>
            <th style="width:48px;">#</th><th>Meta</th>
            <th class="text-center">Beneficiários</th><th>Exec. Física</th>
            <th class="text-right">Exec. Financeira</th><th>Prazo</th><th>Status</th>
          </tr>
        </thead>
        <tbody>
          ${metas.map(m => {
            ...
            return `<tr>
              ...
              <td class="text-center">
                ...
                <span class="text-muted">/${fmt.number(m.beneficiarios_previstos)}</span>
              </td>
              ...
              <td class="text-right">
                ...
                <div style="min-width:60px;">${progressBar(percFin)}</div>
              </td>
